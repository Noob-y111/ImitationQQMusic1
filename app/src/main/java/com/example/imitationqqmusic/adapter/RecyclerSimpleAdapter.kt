...
class RecyclerSimpleAdapter(private val viewModel: MainViewModel, private val clickListener: OnItemClick) : ListAdapter<Album, RecyclerView.ViewHolder>(Compare) {
...
//    var listener: OnErrorClickCallBack? = null
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        when (viewType) {
            FOOTER -> {
                LayoutInflater.from(parent.context).inflate(R.layout.music_footer, parent, false).also {
                    val holder = Footer(it)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            is MyHolder -> {
                ...
                holder.itemView.setOnClickListener {
                    viewModel.setShouldTranslate(true)
                    Navigation.findNavController(it).navigate(R.id.action_musicFragment_to_newOfDayFragment)
                }
//                if (state == null){
